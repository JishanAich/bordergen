...
(function (BG) {
  ...
  // Returns the four side transforms (top,right,bottom,left) mapping local (u,v) to canvas x,y.
  function sideTransforms(W, H, T) {
    return [
      function (u, v) { return [T + u, v]; },
      function (u, v) { return [W - v, T + u]; },
      function (u, v) { return [W - T - u, H - v]; },
      function (u, v) { return [v, H - T - u]; }
    ];
  }
  ...
  BG.buildFrame = function (state) {
    ...
    const names = ['top', 'right', 'bottom', 'left'], tfs = sideTransforms(W, H, T);
    for (let s = 0; s < 4; s++) {
      const L = s % 2 === 0 ? W - 2 * T : H - 2 * T;
      ctx.tp = tfs[s];
      svg.push('<g id="side-' + names[s] + '">');
    ...
    corners.forEach(function (c, i) {
      ...
      svg.push('<rect x="' + c[0] + '" y="' + c[1] + '" width="' + T + '" height="' + T + '" fill="' + pal[style.corner.bg] + '" stroke="' + ruleCol + '" stroke-width="' + ruleH + '"/>');
      svg.push(drawRosetteArt(ctx, c[0] + T / 2, c[1] + T / 2, T * 0.46, style.corner).join(''));
      svg.push('</g>');
    });
